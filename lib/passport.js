...
module.exports = function(configuration) {
  ...
  var fetchUtilSkipSessionCreation = function(req, res, next) {
    if (req.headers && req.headers['user-agent'] && req.headers['user-agent'].indexOf('node-fetch') != -1 && !req.headers.cookie) {
      return next();
    }
    return settings.session(req, res, next)
  };
  ...
  return function() {
    ...
    app.use(settings.cookieParser(settings.secret))
      .use(fetchUtilSkipSessionCreation)
